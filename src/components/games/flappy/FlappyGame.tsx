...
import { Game, GameComponent, GameLogLevels } from "game-reactor/dist";
import type { GameMouseEvent, SystemPerformance } from "game-reactor/dist";
...
const FlappyGameComponent: FC = () => {
  ...
  useEffect(() => {
    setGame(new FlappyGame());
  }, []);

  if (!game) return <div style={{ color: 'white', padding: '2rem' }}>Loading Flappy Jollibee...</div>;
